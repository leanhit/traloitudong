...
// --- Giả lập imageApi cho mục đích demo ---
// Trong thực tế, bạn sẽ thay thế bằng client API thực sự của mình
const imageApi = {
  async addImage(data: any, file?: File) {
    console.log('API: Đang thêm ảnh mới:', data, 'File:', file);
    return new Promise((resolve) => {
      setTimeout(() => {
        if (data.name && (data.url || file)) {
          ElMessage.success('Thêm ảnh thành công (giả lập)!');
          resolve({
            data: { id: Date.now().toString(), ...data, url: file ? 'https://dummyimage.com/150x150/000/fff&text=Uploaded' : data.url },
            message: 'Success'
          });
        } else {
          ElMessage.error('Thêm ảnh thất bại (giả lập): Thiếu tên hoặc URL/File.');
          resolve({ data: null, message: 'Thiếu thông tin' });
        }
      }, 1000);
    });
  },
  async updateImage(id: string, data: any, file?: File) {
    console.log('API: Đang cập nhật ảnh:', id, data, 'File:', file);
    return new Promise((resolve) => {
      setTimeout(() => {
        if (data.name && (data.url || file)) {
          ElMessage.success('Cập nhật ảnh thành công (giả lập)!');
          resolve({
            data: { id: id, ...data, url: file ? 'https://dummyimage.com/150x150/000/fff&text=Updated' : data.url },
            message: 'Success'
          });
        } else {
          ElMessage.error('Cập nhật ảnh thất bại (giả lập): Thiếu tên hoặc URL/File.');
          resolve({ data: null, message: 'Thiếu thông tin' });
        }
      }, 1000);
    });
...
export default {
  ...
  setup(props: any, context: any) {
    ...
    // Danh sách các danh mục mẫu (có thể lấy từ API nếu có)
